...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    def _handle_daily_audit(_now):
        _LOGGER.info(
            "Scheduled daily SensorPush Local audit firing (hour=%d)", daily_hour
        )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok:
        ...
        if coordinator._unsub_daily_audit:  # pylint: disable=protected-access
            coordinator._unsub_daily_audit()  # pylint: disable=protected-access
...
class SensorPushCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        ...
        if self.lock.locked():
            _LOGGER.debug(
                "Audit already in progress — skipping concurrent refresh to preserve existing data."
            )
            return self.data
        ...
        _LOGGER.info(
            "SensorPush Local audit complete — %d/%d device(s) updated",
            updated,
            len(sensor_devices),
        )
            ...
            async with self.lock:
                ...
                try:
                    async with await establish_connection(
                        ...
                        result = await self._read_gatt_chars(
                            client, service_info, ble_device, name
                        )
                        return result
    ...
    async def _read_gatt_chars(self, client, service_info, ble_device, name: str):
        """Read model and battery GATT characteristics and return parsed audit data."""
        ...
        voltage = (
            (float(v_raw) + 2140.0) / 1000.0 if is_legacy else float(v_raw) / 1000.0
        )

        source = (
            service_info.source
            if service_info
            else (ble_device.details or {}).get("source", "unknown")
        )
