...
`prepare-release`'s branch push and PR/auto-merge API calls use the `FORGEJORELEASE_TOKEN` secret — a real Forgejo Personal Access Token (repo write scope), not the default Actions token. This is required for a reason beyond the branch-protection bug: Forgejo never triggers new workflow runs from a push or PR made with the default token (anti-recursion), so `ci.yml`'s required status check would never run on the release branch and auto-merge would wait forever. `FORGEJORELEASE_TOKEN` must be added as a repo secret manually — it can't be generated by the workflow itself. Named without the `FORGEJO_` prefix because Forgejo reserves that prefix for its own secret names.
